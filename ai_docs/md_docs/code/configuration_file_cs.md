...
By default, the set of default configuration files is created automatically on [project creation](../sdk/projects/index_cs.md#creation) in the project's `data/configs` folder available as a separate `configs` section of the Asset Browser.
...
> **Notice:** If the configuration files are stored in a [`*.ung` package](../principles/filesystem/index_cs.md#types), they won't be loaded and, therefore, the engine will be initialized with the default settings. However, when the file system is initialized, you can specify the path to the configuration files stored in the package via the set of corresponding console commands:
...
To read values from the configuration file and write them back, use functionality of the *[BootConfig](../api/library/engine/class.bootconfig_cs.md)* class.
...
To read values from the configuration file and write them back, use functionality of the *[Config](../api/library/engine/class.config_cs.md)* class. Use the appropriate methods depending on the type of the target item.
...
> **Notice:** The *[ControlsApp](../api/library/controls/class.controlsapp_cs.md)* class used to manage this configuration is considered deprecated - for handling input in new projects, use the *[Input](../api/library/controls/class.input_cs.md)* class instead. The `.controls` file itself remains in use.
...
- **keys** - keyboard key bindings for the *[STATE_*](../api/library/controls/class.controls_cs.md#STATE_FORWARD)* states of the *Controls* class. Each child element is named after the state (FORWARD, BACKWARD, etc.) and contains the bound key name. Unassigned states have the UNKNOWN value.
- **mouse_buttons** - mouse button bindings for the same *[STATE_*](../api/library/controls/class.controls_cs.md#STATE_FORWARD)* states.


To manage the controls configuration, use functionality of the *[ControlsApp](../api/library/controls/class.controlsapp_cs.md)* class or classes derived from the *[Controls](../api/library/controls/class.controls_cs.md)* class.
...
- The **render** section stores the declarations of custom parameters and the values of the ones that belong to the project as a whole:
  ...
  - **surface_custom_parameters** and **material_custom_parameters** - [declarations of custom surface and material parameters](../content/materials/custom_parameters/declaring_and_setting_cs.md#declare_storage). Each element is named after the parameter and keeps its GUID, type, default value, and the range of values. Only the declarations are stored here: values of a surface are saved in the world, values of a material - in its `*.mat` file.
...
To control the global configuration file, use functionality of the *[GlobalConfig](../api/library/engine/class.globalconfig_cs.md)* class.
...
To control the user configuration file, use functionality of the *[UserConfig](../api/library/engine/class.userconfig_cs.md)* class.
