# Tracking Hands and Fingers With Ultraleap Plugin (USC)
...
```cpp
Mat4 camera_imodelview = camera.getIModelview();
Mat4 camera_offset = Mat4(camera.getOffset());

engine.ultraleap.setTransform(camera_imodelview * camera_offset);

```


> **Notice:** Depending on your application, you might require to manually provide correct HMD Position that was updated during the plugin update **before the plugin swap event** using the [BeginSwap](../../../api/library/engine/class.engine_usc.md#getEventBeginSwap_Event) event (native VR plugins already do this internally).
...
Hands are represented by the *[Hand](../../../api/library/plugins/ultraleap/class.ultraleaphand_usc.md)* class.
...
Arms are represented by the *[Arm](../../../api/library/plugins/ultraleap/class.ultraleaparm_usc.md)* class.
...
Fingers are represented by the *[Finger](../../../api/library/plugins/ultraleap/class.ultraleapfinger_usc.md)* class.
...
Bones are represented by the *[Bone](../../../api/library/plugins/ultraleap/class.ultraleapbone_usc.md)* class.
...
- The *[Ultraleap](../../../api/library/plugins/ultraleap/class.ultraleap_usc.md)* interface article for more details on managing Ultraleap via API
- The *[Ultraleap](../../../api/library/plugins/ultraleap/class.ultraleapdevice_usc.md)* interface article for more details on managing Ultraleap devices via API
- The *[Ultraleap Arm](../../../api/library/plugins/ultraleap/class.ultraleaparm_usc.md)* class article for more details on managing arms via API
- The *[Ultraleap Bone](../../../api/library/plugins/ultraleap/class.ultraleapbone_usc.md)* class article for more details on managing finger bones via API
- The *[Ultraleap Finger](../../../api/library/plugins/ultraleap/class.ultraleapfinger_usc.md)* class article for more details on managing fingers via API
- The *[Ultraleap Hand](../../../api/library/plugins/ultraleap/class.ultraleaphand_usc.md)* class article for more details on managing hands via API
...
2. To **add the plugin to a new project**, start by [creating a project](../../../sdk/projects/index.md#creation) from a template. In the project creation dialog, open *Advanced Settings > Plugins*, enable the *Ultraleap* plugin, click *Add*, then select *Create New Project*. ![](add_plugin.png) For **existing projects**, in the SDK Browser, open the *My Projects* tab, and click the three-dot menu on the project card. Select *Configure*, then click *Plugins*, enable the required plugin, click *Add*, and finish with *Configure Project*. ![](../../../sdk/projects/other_actions.png)
...
If you run the application via UNIGINE SDK Browser, specify the command-line options given above in the *[Customize Run Options](../../../sdk/projects/index.md#customize_run)* form.
