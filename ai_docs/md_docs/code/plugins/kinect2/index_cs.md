...
- *[Kinect Class (engine.kinect)](../../../api/library/plugins/engine.kinect_cs.md)* functions
...
To **add the plugin to a new project**, start by [creating a project](../../../sdk/projects/index_cs.md#creation) from a template. In the project creation dialog, open *Advanced Settings > Plugins*, enable the *Kinect2* plugin, click *Add*, then select *Create New Project*.
...
- Buffers: [color](../../../api/library/plugins/engine.kinect_cs.md#STREAM_COLOR), [depth](../../../api/library/plugins/engine.kinect_cs.md#STREAM_DEPTH), [IR range](../../../api/library/plugins/engine.kinect_cs.md#STREAM_INFRARED).
...
- Faces (up to 6 faces can be detected): face [boundaries](../../../api/library/plugins/engine.kinect_cs.md#getFaceBoundsInColorSpace_int_ivec4) and facial key [points](../../../api/library/plugins/engine.kinect_cs.md#getFacePointInColorSpace_int_int_vec3) (eyes, mouth, nose) in coordinates of color and IR buffers, features (eye glasses, smile, closed eyes).


> **Notice:** When the plugin is loaded, the *[engine.kinect](../../../api/library/plugins/engine.kinect_cs.md)* library with a bunch of functions is added to UnigineScript.


When implementing an application using the plugin, it is necessary to call the *[Init()](../../../api/library/plugins/engine.kinect_cs.md#init_uint_int)* function with the required arguments on engine initialization and the *[Shutdown()](../../../api/library/plugins/engine.kinect_cs.md#shutdown_void)* function on engine shutdown. For example:
