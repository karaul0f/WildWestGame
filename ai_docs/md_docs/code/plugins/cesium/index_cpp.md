...
The anchor is owned by *[Geodetics](../../../code/plugins/geodetics/index_cpp.md)* and driven by *Cesium*. Everything that lives in the local frame - entities, cameras, weather - must recompute its position from geodetic coordinates whenever the anchor moves, by subscribing to the anchor-changed event - *[Geodetics::Anchor::getEventChanged()](../../../api/library/geodetics/geodetics_plugin/class.anchor_cpp.md#EventChanged)*. In an IG application this is already handled: IG places its entities and cameras through the anchor, and Weather repositions the sky, sun and clouds on the same event.
...
```cpp
#include <plugins/Unigine/Cesium/UnigineCesium.h>
#include <plugins/Unigine/Geodetics/UnigineGeodetics.h>

using namespace Unigine;

void AppSystemLogic::init_cesium()
{
    auto *cesium = Plugins::Cesium::get();
    if (!cesium)
        return;

    // Cesium drives the anchor, so the world scan must not stamp a static
    // georeference over it on world load.
    auto *geodetics = Plugins::Geodetics::Converter::get();
    geodetics->setAutoWorldInit(false);
    ...
    if (ig_manager && !ig_manager->isMaster())
        cesium->setAnchorAutoUpdate(false);
}
...
{
    if (auto *cesium = Unigine::Plugins::Cesium::get())
    {
        auto *ig = Unigine::Plugins::IG::Manager::get();
        Unigine::Plugins::IG::View *view = ig ? ig->getView(ig->getCurrentView(), false) : nullptr;
        ...
        Unigine::PlayerPtr player = view && view->isEnabled() ? view->getPlayer() : Unigine::PlayerPtr();
        cesium->setPlayer(player);
...
```cpp
const double altitude = max(spectator.getAltitude(), 0.0);
const float zfar = float(max(VIEW_FAR_FLOOR_M, VIEW_FAR_MARGIN
    * sqrt(2.0 * Geodetics::Converter::WGS84_MAJOR_AXIS * altitude + altitude * altitude)));
const float znear = max(1.0f, zfar / VIEW_NEAR_FAR_RATIO);
...
|---|---|
| Supply a player every frame via *[setPlayer()](../../../api/library/plugins/cesium/class.cesium_cpp.md#Player)*. | The plugin has no camera, the anchor never moves, and nothing rebases. |
| Call *[setAnchorAutoUpdate(false)](../../../api/library/plugins/cesium/class.cesium_cpp.md#AnchorAutoUpdate)* on a follower, such as a [Syncker](../../../code/plugins/syncker/index.md) slave. | The follower runs its own rebase logic and fights the anchor set externally. |
| Call *[setAutoWorldInit(false)](../../../api/library/geodetics/geodetics_plugin/class.converter_cpp.md#AutoWorldInit)* on the *Geodetics* converter, and only when Cesium drives the anchor. | Loading a world overwrites the anchor with a static georeference. Conversely, an application without Cesium that disables it never georeferences its worlds at all. |
| Establish the starting frame yourself once the world scan has been switched off - *[reset()](../../../api/library/geodetics/geodetics_plugin/class.anchor_cpp.md#reset_void)* on the *Geodetics* anchor declares the orbital one. | Nothing defines a frame, since the scan is what normally does it, and a start world with no georeferenced terrain has nothing to define one from. The plugin then streams nothing at all. |
| Open the frustum up to planetary distances - in an IG application through the *[*IG::View*](../../../api/library/plugins/ig/api/class.view_cpp.md)* definition rather than on the player. | The globe is clipped away when seen from orbit, so the far plane has to reach thousands of kilometres while the near plane stays at a metre or more for the depth buffer to remain usable at that range. Setting the two on the player every frame additionally throws IG's own frustum away, because *[Camera](../../../api/library/rendering/class.camera_cpp.md)* rebuilds its projection at a 1:1 aspect. |


## Applying Changes and Reading State


The configuration is available at run time through *[Cesium::getConfig()](../../../api/library/plugins/cesium/class.cesium_cpp.md#getConfig_CesiumConfig)*. After editing it, call *[applyConfig()](../../../api/library/plugins/cesium/class.cesium_cpp.md#applyConfig_bool)* for the change to take effect. Settings the tilesets read (access token, asset list, **streaming**, **tile_masks**, **cache**) rebuild them, so every loaded tile is dropped and the globe is streamed again; the rest (inset list and log level) are applied at once. Call *[save()](../../../api/library/plugins/cesium/class.cesiumconfig_cpp.md#save_bool)* on the configuration first to make the change persistent - omit it for a change that should die with the application.


```cpp
auto *config = cesium->getConfig();

// addAsset() appends an empty entry and returns its index; addInset() does the same
const int index = config->addAsset();
config->setAssetName(index, "OSM Buildings");
config->setAssetType(index, Plugins::CesiumConfig::ASSET_TYPE_TILES_3D);
config->setAssetSource(index, Plugins::CesiumConfig::ASSET_SOURCE_CESIUM_ION);
config->setAssetIonId(index, 96188);
config->setAssetEnabled(index, true);

config->save();                                  // persist to the configuration file
const bool streaming = cesium->applyConfig();     // rebuild the tilesets

```


Streaming statistics are available as *[getNumRenderTiles()](../../../api/library/plugins/cesium/class.cesium_cpp.md#NumRenderTiles)*, *[getNumLoadedTiles()](../../../api/library/plugins/cesium/class.cesium_cpp.md#NumLoadedTiles)*, *[getNumLoadingTiles()](../../../api/library/plugins/cesium/class.cesium_cpp.md#NumLoadingTiles)*, *[getLoadProgress()](../../../api/library/plugins/cesium/class.cesium_cpp.md#LoadProgress)* and *[getUsedMemoryMb()](../../../api/library/plugins/cesium/class.cesium_cpp.md#UsedMemoryMb)*. Three more report what the traversal did rather than what is on screen: *[getNumVisitedTiles()](../../../api/library/plugins/cesium/class.cesium_cpp.md#NumVisitedTiles)*, *[getNumCulledTiles()](../../../api/library/plugins/cesium/class.cesium_cpp.md#NumCulledTiles)* and *[getMaxTileDepth()](../../../api/library/plugins/cesium/class.cesium_cpp.md#MaxTileDepth)*.


> **Notice:** *[isStreaming()](../../../api/library/plugins/cesium/class.cesium_cpp.md#IsStreaming)* reports that the globe has what it needs and its tilesets are built. It does not report that tiles are arriving - that can only be judged from the tile counters, since the plugin is ready before a world exists and before the first tile has been requested.


The debug overlay - the tile bounding volumes and the inset bounds - is a separate case. It is not a part of the [configuration](../../../code/plugins/cesium/config.md) and is never saved: *[renderTileBounds()](../../../api/library/plugins/cesium/class.cesium_cpp.md#renderTileBounds_int_void)* requests it for one frame at a time, so the application calls it every frame while the bounds should stay on screen, and stops calling it to hide them. Drawing goes through the engine visualizer, which the application has to enable itself:
...
The [terrain insets](../../../code/plugins/cesium/insets.md) listed in the configuration are registered when the plugin is initialized and re-registered whenever the configuration is applied, and are addressed by index: *[getNumInsets()](../../../api/library/plugins/cesium/class.cesium_cpp.md#NumInsets)*, *[getInsetNode()](../../../api/library/plugins/cesium/class.cesium_cpp.md#getInsetNode_int_Node)*, *[getInsetGeoPosition()](../../../api/library/plugins/cesium/class.cesium_cpp.md#getInsetGeoPosition_int_dvec3_bool)* and *[getInsetStatus()](../../../api/library/plugins/cesium/class.cesium_cpp.md#getInsetStatus_int_int)*.


A registered inset is not necessarily a usable one: the world may be unreadable, or its terrain may not meet the [requirements](../../../code/plugins/cesium/insets.md). *[getInsetStatus()](../../../api/library/plugins/cesium/class.cesium_cpp.md#getInsetStatus_int_int)* answers which of the two it is, as one of the *[INSET_STATUS_*](../../../api/library/plugins/cesium/class.cesium_cpp.md#INSET_STATUS_OK)* values.


The same check is available for a world or a node that has not been registered - *[checkInsetWorld()](../../../api/library/plugins/cesium/class.cesium_cpp.md#checkInsetWorld_cstr_int)* and *[checkInsetNode()](../../../api/library/plugins/cesium/class.cesium_cpp.md#checkInsetNode_Node_int)* - which is what the [editor panel](../../../code/plugins/cesium/editor_plugin.md) reports beside an inset in its list.


Activation follows the camera, and can also be driven by the application: *[findNearbyInset()](../../../api/library/plugins/cesium/class.cesium_cpp.md#findNearbyInset_dvec3_int)* returns the inset registered near a geodetic position, *[setActiveInset()](../../../api/library/plugins/cesium/class.cesium_cpp.md#setActiveInset_int_bool)* swaps one in by index, *[setCustomActiveInset()](../../../api/library/plugins/cesium/class.cesium_cpp.md#setCustomActiveInset_Node_bool)* does the same for a terrain node of your own, and *[clearActiveInset()](../../../api/library/plugins/cesium/class.cesium_cpp.md#clearActiveInset_void)* drops back to the streamed globe. *[hasActiveInset()](../../../api/library/plugins/cesium/class.cesium_cpp.md#hasActiveInset_bool)*, *[isInsetActive()](../../../api/library/plugins/cesium/class.cesium_cpp.md#isInsetActive_int_bool)* and *[getActiveInsetIndex()](../../../api/library/plugins/cesium/class.cesium_cpp.md#ActiveInsetIndex)* report the current state.
...
| One active inset | Only one [terrain inset](../../../code/plugins/cesium/insets.md) can be swapped in at a time. It is chosen by camera proximity, and can also be [set by the application](../../../code/plugins/cesium/index_cpp.md#insets_api). |
...
| Haze disabled by default | Flat-world haze draws as a hard band across the horizon on a globe. It can be re-enabled in the [weather configuration](../../../ig/weather/config_cpp.md). |
