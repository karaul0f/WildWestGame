...
Outputs the angle whose cosine is **A**.


Angles are measured in radians.


The input is clamped to the range from -1 to 1, so a value outside it gives the nearest valid angle rather than an error.
