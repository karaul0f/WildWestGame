...
An event does nothing until game code handles it. You add one callback to the [player](../../../../../editor2/tools/sequencer/runtime/index_cs.md) running the file, and it receives *every* event that player fires - the callback is not tied to a single name. Each call gives you three values:
...
public class EventLogger : Component
{
    ...
    private void Init()
    {
        ...
        NodeSequencePlayer playerNode = node as NodeSequencePlayer;
        playerNode.GetPlayer().EventTriggered.Connect(connections, OnEvent);
    }

    // Called once for every event this player fires - filter by name here.
    private void OnEvent(string name, int payload, int phase, string placement)
    {
        if (name == "footstep")
            Log.Message($"Footstep! payload {payload}\n");
    }
}
...
- [Runtime Playback](../../../../../editor2/tools/sequencer/runtime/index_cs.md)
