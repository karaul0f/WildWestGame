...
The settings that only apply when the world is rendered on a round planet � as it is with the *[Cesium](../../code/plugins/cesium/index_cpp.md)* plugin � are added to the variables group with the name **Planet**. On a flat map none of them has any effect.
...
- **enabled** � switch on the round-planet corrections: the altitude-driven cloud rounding, the sky offset that holds the horizon still across a [floating-origin rebase](../../code/plugins/cesium/index_cpp.md#anchor), the haze veto and the cloud noise continuity fixup
...
The values in force at run time can be read back through the *[Weather::Manager::getPlanet()](../../api/library/plugins/weather/class.weather_manager_cpp.md#Planet)* interface: *[isEnabled()](../../api/library/plugins/weather/class.planet_cpp.md#IsEnabled)*, *[isHazeEnabled()](../../api/library/plugins/weather/class.planet_cpp.md#IsHazeEnabled)* and *[getCloudsRadius()](../../api/library/plugins/weather/class.planet_cpp.md#CloudsRadius)*.
