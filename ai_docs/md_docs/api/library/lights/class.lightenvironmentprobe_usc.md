# Unigine::LightEnvironmentProbe Class (USC)
...
- *int* **mode** - The grabbing mode to be set for the *Environment Probe*. One of the *GRAB_MODE* values.
...
- *string* **path** - The path to a cube texture file.
...
- *int* **enabled** - The ambient (Indirect Diffuse) lighting for the *Environment Probe*

## int isAmbientEnabled () const

Returns the current value indicating if ambient (Indirect Diffuse) lighting is enabled for the *Environment Probe*.
### Return value

Current ambient (Indirect Diffuse) lighting for the *Environment Probe*
...
- *int* **enabled** - The specular reflections for the *Environment Probe*

## int isSpecularEnabled () const

Returns the current value indicating if specular reflections are enabled for the *Environment Probe*.
### Return value

Current specular reflections for the *Environment Probe*
...
## void setGrabGGXMipmapsQuality ( )

Sets a new quality of GGX mipmaps.
### Arguments

- **quality** - The quality of GGX mipmaps, one of the *[GGX_MIPMAPS_QUALITY](../../../api/library/rendering/class.render_usc.md#GGX_MIPMAPS_QUALITY)*values.

## getGrabGGXMipmapsQuality () const

Returns the current quality of GGX mipmaps.
### Return value

Current quality of GGX mipmaps, one of the *[GGX_MIPMAPS_QUALITY](../../../api/library/rendering/class.render_usc.md#GGX_MIPMAPS_QUALITY)*values.
## void setGrabSupersampling ( )

Sets a new supersampling mode for image grabbing.
### Arguments

- **supersampling** - The supersampling mode to be set, one of the GRAB_SUPERSAMPLING values.

## getGrabSupersampling () const
...
## void setGrabResolution ( )

Sets a new resolution of the reflection mask in pixels.
### Arguments

- **resolution** - The resolution of the reflection mask in pixels, one of the GRAB_RESOLUTION values.

## getGrabResolution () const
...
## void setGrabDynamicFacesPerFrame ( )

Sets a new update interval set for the cube texture used for dynamic reflections.
### Arguments

- **frame** - The value indicating the current update interval for the dynamic cube texture (faces per frame).

## getGrabDynamicFacesPerFrame () const
...
- *vec3* **size** - The *Environment Probe* box size along X, Y, and Z axes.
...
Sets a new value indicating if clipping of reflections occluded by obstacles is enabled. this feature uses the [depth texture](../../../api/library/lights/class.light_usc.md#getBakedDepthTextureFilePath_cstr) grabbed for the *Environment Probe* to determine reflections that should be visible.
### Arguments
...
Returns the current value indicating if clipping of reflections occluded by obstacles is enabled. this feature uses the [depth texture](../../../api/library/lights/class.light_usc.md#getBakedDepthTextureFilePath_cstr) grabbed for the *Environment Probe* to determine reflections that should be visible.
### Return value
...
- **mode** - The projection mode to be used for the *Environment Probe*, one of the *PROJECTION_MODE* values.
...
- *vec3* **distance** - The distance from the light source shape, at which the light source doesn't illuminate anything.
...
## void setRaymarchingSpecularBRDF ( )
...
- **brdf** - The light distribution model for matte surfaces, one of the *[SPECULAR_BRDF_MODE](#SPECULAR_BRDF_MODE)* values. GGX is more realistic, though increases noise and slightly reduces performance.

## getRaymarchingSpecularBRDF () const
...
## void setRaymarchingLastStepMode ( )
...
- **mode** - The cubemap for the last step. One of the *[LAST_STEP_MODE](#LAST_STEP_MODE)* values.

## getRaymarchingLastStepMode () const
...
## void setRaymarchingSecondaryBounceProjectionMode ( )

Sets a new secondary bounce projection mode.
### Arguments

- **mode** - The secondary bounce projection mode. One of the *[SECONDARY_BOUNCE_PROJECTION_MODE](#SECONDARY_BOUNCE_PROJECTION_MODE)* values.

## getRaymarchingSecondaryBounceProjectionMode () const
...
## void setGrabDynamicReprojection ( int reprojection )
...
- *int* **reprojection** - The reprojection mode of the dynamic grab

## int getGrabDynamicReprojection () const
...
## void setGrabDynamicInterleaved ( int interleaved )
...
- *int* **interleaved** - The interleaved rendering mode of the dynamic grab

## int getGrabDynamicInterleaved () const

Returns the current interleaved rendering mode for the cube map faces updated by the dynamic grab, one of the *GRAB_DYNAMIC_INTERLEAVED_** values: each updated face is rendered at a reduced resolution and scattered into its own subset (phase) of the face texels, spreading the full-resolution refresh over several updates and reducing the per-update cost accordingly. Disabled by default.
### Return value

Current interleaved rendering mode of the dynamic grab
## void setGrabDynamicInterleavedColorClamping ( int clamping )
...
- *int* **clamping** - The color clamping mode of the interleaved dynamic grab

## int getGrabDynamicInterleavedColorClamping () const
...
[LightEnvironmentProbe](../../../api/library/nodes/class.node_usc.md#LIGHT_ENVIRONMENT_PROBE) type identifier.
