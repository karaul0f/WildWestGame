...
Returns the current size of the area illuminated by the light source. depending on the [shape type](#setShapeType_int_void), the size varies:
...
- *const [Ptr](../../../api/library/common/class.ptr_cpp.md)<[Image](../../../api/library/common/class.image_cpp.md)> &* **image** - New texture to set.
