# Unigine.FieldWeather Class (USC)
...
This class is used to create and modify a [weather field](../../../objects/effects/fields/field_weather/index.md). The field is applied to [CloudLayer object](../../../api/library/objects/class.objectcloudlayer_usc.md) and specifies a local weather area. This type of field makes it possible to create local storms or clouds having their own coverage texture as well as to control their movement.
...
- *vec3* **offset** - The coverage texture offset. A [vec3](../../../api/library/math/class.vec3_usc.md) vector with components representing the offset values of the coverage texture along the **X**, **Y** and **Z** axes.

## vec3 getCloudsAnimationOffset () const

Returns the current offset of the FieldWeather coverage texture.
### Return value

Current coverage texture offset. A [vec3](../../../api/library/math/class.vec3_usc.md) vector with components representing the offset values of the coverage texture along the **X**, **Y** and **Z** axes.
...
## void setWindAnimation ( vec3 animation )
...
- *vec3* **animation** - The local wind speed [vec3](../../../api/library/math/class.vec3_usc.md) vector with components representing the wind speed values in the **X**, **Y** and **Z** directions.

## vec3 getWindAnimation () const
...
Current local wind speed [vec3](../../../api/library/math/class.vec3_usc.md) vector with components representing the wind speed values in the **X**, **Y** and **Z** directions.
...
- *[Texture](../../../api/library/rendering/class.texture_usc.md)* **texture** - The texture instance.
...
- *string* **path** - The path to the coverage texture.
...
- *vec3* **size** - The FieldWeather size. A [vec3](../../../api/library/math/class.vec3_usc.md) vector with components representing the sizes along the **X**, **Y** and **Z** axes, in units.
...
Current FieldWeather size. A [vec3](../../../api/library/math/class.vec3_usc.md) vector with components representing the sizes along the **X**, **Y** and **Z** axes, in units.
---
...
- *[Image](../../../api/library/common/class.image_usc.md)* **image** - Image instance with a coverage texture for the FieldWeather.
...
- *[Image](../../../api/library/common/class.image_usc.md)* **image** - Image instance into which the current coverage texture will be grabbed.
...
[FieldWeather](../../../api/library/nodes/class.node_usc.md#FIELD_WEATHER) type identifier.
