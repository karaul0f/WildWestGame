# Unigine::Plugins::CesiumConfig Class (CPP)
...
| Name | Description |
|---|---|
| **ASSET_TYPE_TERRAIN** = 0 | Elevation data forming the ground surface. Accepts imagery overlays, the water mask, and terrain insets. At most one terrain asset is used: if several are enabled, the first one is taken and the rest are ignored with a warning. |
| **ASSET_TYPE_IMAGERY** = 1 | A raster layer draped over the ground surface. Requires an enabled terrain asset, otherwise it is ignored. Up to four overlays are composited in the order they are listed, a later overlay drawn over an earlier one. |
| **ASSET_TYPE_TILES_3D** = 2 | Pre-textured geometry (photorealistic city models, buildings, scans). Carries its own textures and takes no imagery; can be used as the only source of geometry. |
...
| Name | Description |
|---|---|
| **ASSET_SOURCE_CESIUM_ION** = 0 | The tiles are streamed from *[Cesium ion](https://cesium.com/platform/cesium-ion/)* by the asset ID. Requires an access token entitled to the asset. |
| **ASSET_SOURCE_CUSTOM** = 1 | A direct tileset URL or a local path, intended for isolated networks. Not implemented yet: such an asset is skipped and the reason is reported to the log. |
...
| Name | Description |
|---|---|
| **LOG_LEVEL_ERROR** = 0 | Failures only. |
| **LOG_LEVEL_WARNING** = 1 | Errors plus recoverable problems, such as an invalid configuration or unsupported data. |
| **LOG_LEVEL_INFO** = 2 | Warnings plus normal lifecycle messages (default). |
| **LOG_LEVEL_DEBUG** = 3 | Info plus developer detail: inset registration and activation, inset bounds, per-tile information. This level also reveals rejected tokens, quota, and entitlement issues reported by the network layer. |

### Members

## bool isIonAccessTokenFromEnvironment () const
...
**true** if the ion access token was taken from the environment; otherwise **false**.
...
- *const char ** **token** - *Cesium ion* access token.

## const char * getIonAccessTokenPathOverride ( ) const
...
## void setIonAccessTokenPathOverride ( const char * path )

Sets the override path to the file that holds the *Cesium ion* access token.
### Arguments

- *const char ** **path** - Override path to the token file; an empty string means the default location.

## const char * getResolvedIonAccessTokenPath ( ) const
...
## const char * getDefaultIonAccessTokenPath ( ) const
...
Name of the asset.
## void setAssetName ( int index , const char * name )

Sets the name of the asset with the given index, displayed in the user interface and the log.
### Arguments

- *int* **index** - Index of the asset.
- *const char ** **name** - Name of the asset.

## CesiumConfig::ASSET_TYPE getAssetType ( int index ) const
...
- *int* **index** - Index of the asset.
- *[CesiumConfig::ASSET_TYPE](../../../../api/library/plugins/cesium/class.cesiumconfig_cpp.md#ASSET_TYPE)* **type** - Type of the asset, one of the *ASSET_TYPE_** values.

## CesiumConfig::ASSET_SOURCE getAssetSource ( int index ) const
...
- *int* **index** - Index of the asset.
- *[CesiumConfig::ASSET_SOURCE](../../../../api/library/plugins/cesium/class.cesiumconfig_cpp.md#ASSET_SOURCE)* **source** - Source of the asset, one of the *ASSET_SOURCE_** values.

## int getAssetIonId ( int index ) const
...
## bool isAssetEnabled ( int index ) const
...
## const char * getInsetName ( int index ) const
...
Name of the inset.
## void setInsetName ( int index , const char * name )

Sets the name of the terrain inset with the given index, displayed in the user interface and the log.
### Arguments

- *int* **index** - Index of the inset.
- *const char ** **name** - Name of the inset.

## const char * getInsetPath ( int index ) const
...
## void setInsetPath ( int index , const char * path )

Sets the path to the world file of the terrain inset with the given index.
### Arguments

- *int* **index** - Index of the inset.
- *const char ** **path** - Path to the inset world file, relative to the data directory of the project.

## bool isInsetEnabled ( int index ) const
...
## double getInsetActivationDistance ( int index ) const
...
## double getMaxScreenSpaceError ( ) const
...
## double getCulledScreenSpaceError ( ) const
...
## bool isFogCullingEnabled ( ) const
...
## bool isTerrainWaterMaskEnabled ( ) const
...
## int getTileIntersectionMask ( ) const
...
## int getTileCollisionMask ( ) const
...
## int getTilePhysicsIntersectionMask ( ) const
...
## double getTileCacheUnloadTimeLimitMs ( ) const
...
## CesiumConfig::LOG_LEVEL getLogLevel ( ) const
...
- *[CesiumConfig::LOG_LEVEL](../../../../api/library/plugins/cesium/class.cesiumconfig_cpp.md#LOG_LEVEL)* **level** - Verbosity level, one of the *LOG_LEVEL_** values.
