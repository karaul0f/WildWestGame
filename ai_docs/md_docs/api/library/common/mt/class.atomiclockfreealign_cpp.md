...
Implicitly converts the **AtomicLockFreeAlign** object to a value of type **Type**. Internally calls *[fetch()](#fetch_Type)* to return a thread-safe snapshot of the current value.
### Return value
...
- *const AtomicLockFreeAlign&* **v** - Another atomic to copy from.
