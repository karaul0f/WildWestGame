# Unigine::CustomParameterLayout Class (CS)


The *CustomParameterLayout* class represents a global layout (schema) of custom parameters: an ordered list of typed parameters, each having a name, a GUID, a default value, and an editor slider range. Two layouts exist in the engine: the *[surface parameters layout](../../../api/library/rendering/class.render_cs.md#getSurfaceParameters_CustomParameterLayout)*, each parameter of which exists on every surface of every object and on every decal, and the *[material parameters layout](../../../api/library/rendering/class.materials_cs.md#getMaterialParameters_CustomParameterLayout)*, each parameter of which exists on every material. Instances of this class are never created manually: the two layouts are obtained from the corresponding singletons.
...
Both layouts can be defined in UnigineEditor (the *Custom Parameters* section of the render settings) or via this API; the schemas are stored in the engine global config. Changing the schema at runtime triggers the *[EventChangedParameters](../../../api/library/rendering/class.render_cs.md#getEventChangedParameters_Event)* event.
...
| Name | Description |
|---|---|
| **FLOAT** = 0 | Floating-point parameter type. |
| **INT** = 1 | Signed integer parameter type. |
| **UINT** = 2 | Unsigned integer parameter type. |

### Properties

## 🔒︎ int NumParameters

The number of custom parameters in the layout.
## 🔒︎ int StructureSize

The size of the GPU-side structure storing the parameter values of a single owner, in bytes. The size includes the engine builtin fields and 4 bytes per custom parameter, rounded up to a multiple of 16 bytes (the minimum size is 16 bytes).
### Members
...
- *[UGUID](../../../api/library/filesystem/class.uguid_cs.md)* **guid** - Parameter GUID.
...
## CustomParameterLayout.TYPE GetParameterType ( int num )
...
- *int* **num** - Parameter number.
- *[CustomParameterLayout.TYPE](../../../api/library/common/class.customparameterlayout_cs.md#TYPE)* **type** - New parameter type, one of the *TYPE_** values.
...
Adds a new custom parameter of the floating-point type to the layout. The name must be a valid parameter name (see *[isValidSurfaceMaterialParameterName()](../../../api/library/rendering/class.render_cs.md#isValidSurfaceMaterialParameterName_cstr_int)*).
### Arguments
...
Adds a new custom parameter of the signed integer type to the layout. The name must be a valid parameter name (see *[isValidSurfaceMaterialParameterName()](../../../api/library/rendering/class.render_cs.md#isValidSurfaceMaterialParameterName_cstr_int)*).
### Arguments
...
Maximum parameter value, or 0 if the number is out of range.
## uint AddParameterUInt ( string name , uint default_value , UGUID guid = UGUID() )

Adds a new custom parameter of the unsigned integer type to the layout. The name must be a valid parameter name (see *[isValidSurfaceMaterialParameterName()](../../../api/library/rendering/class.render_cs.md#isValidSurfaceMaterialParameterName_cstr_int)*).
### Arguments

- *string* **name** - Parameter name.
- *uint* **default_value** - Default parameter value.
- *[UGUID](../../../api/library/filesystem/class.uguid_cs.md)* **guid** - Parameter GUID. If an empty GUID is passed, a new one is generated automatically.

### Return value

Number of the added parameter, or -1 if the parameter cannot be added (the name is invalid, or a parameter with the same name or GUID already exists).
## void SetParameterDefaultUInt ( int num , uint value )

Sets a new default value for the custom parameter with the given number. The default value applies wherever the parameter is not overridden explicitly.
### Arguments

- *int* **num** - Parameter number.
- *uint* **value** - New default parameter value.

## uint GetParameterDefaultUInt ( int num )
...
Default parameter value, or 0 if the number is out of range.
## void SetParameterMinUInt ( int num , uint value )

Sets a new minimum value for the custom parameter with the given number. The minimum and maximum define the slider range for the parameter in UnigineEditor and are not enforced by the engine (values outside the range are not clamped).
### Arguments

- *int* **num** - Parameter number.
- *uint* **value** - New minimum parameter value.

## uint GetParameterMinUInt ( int num )
...
Minimum parameter value, or 0 if the number is out of range.
## void SetParameterMaxUInt ( int num , uint value )

Sets a new maximum value for the custom parameter with the given number. The minimum and maximum define the slider range for the parameter in UnigineEditor and are not enforced by the engine (values outside the range are not clamped).
### Arguments

- *int* **num** - Parameter number.
- *uint* **value** - New maximum parameter value.

## uint GetParameterMaxUInt ( int num )
