# Unigine::ExperimentalNavigationMeshFilter Class (USC)
...
Returns the current mask matched against the obstacle masks of the [Obstacle](../../../api/library/pathfinding/class.obstacle_usc.md) nodes that the query has to avoid. An obstacle is taken into account only when its own mask shares at least one bit with this one. The value 0 makes the query ignore obstacles altogether, which leaves detouring around them entirely to local avoidance.
### Return value

Current obstacle mask, an integer value. The default value is 1.
## void setSnapSize ( vec3 size )
...
- *vec3* **size** - The full size of the search box along each axis, in units. Negative components are clamped to 0. The default value is (1, 1, 1).

## vec3 getSnapSize () const
...
## static ExperimentalNavigationMeshFilter ( )
...
Excluded obstacle.
## int isAreaCostOverridden ( int area_index )
...
- *[Obstacle](../../../api/library/pathfinding/class.obstacle_usc.md)* **obstacle** - Obstacle to be taken into account again.
