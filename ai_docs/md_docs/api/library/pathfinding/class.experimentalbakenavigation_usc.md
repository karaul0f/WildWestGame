# Unigine::ExperimentalBakeNavigation Class (USC)
...
Builds navigation meshes out of the geometry of the scene. Baking voxelizes everything the bake mask of a [navigation mesh](../../../api/library/pathfinding/class.experimentalnavigationmesh_usc.md) selects, works out which surfaces an agent of the configured size can stand and walk on, and turns the result into convex polygons arranged in tiles.
...
- **Async** � spreads the tiles over worker threads and hands back an [ExperimentalNavigationBakeQuery](../../../api/library/pathfinding/class.experimentalnavigationbakequery_usc.md) to watch.
...
## ExperimentalNavigationBakeQuery engine.bake_navigation. bakeAsync ( ExperimentalNavigationMesh navigation_mesh )

Starts baking a navigation mesh on the workers and returns at once.
### Arguments

- *[ExperimentalNavigationMesh](../../../api/library/pathfinding/class.experimentalnavigationmesh_usc.md)* **navigation_mesh** - Navigation mesh to be baked.

### Return value

Query to watch the bake with.
## ExperimentalNavigationBakeQuery engine.bake_navigation. bakeAsync ( ExperimentalNavigationMesh [] navigation_meshes )
...
- *[ExperimentalNavigationMesh](../../../api/library/pathfinding/class.experimentalnavigationmesh_usc.md)[]* **navigation_meshes** - Navigation meshes to be baked.

### Return value

Query covering the whole batch.
## ExperimentalNavigationBakeQuery engine.bake_navigation. bakeAsync ( NavigationMesh navigation_mesh )

Starts baking a [NavigationMesh](../../../api/library/pathfinding/class.navigationmesh_usc.md) node on the workers and returns at once. The surface is built by the same pipeline and then converted into the triangle mesh that node navigates over, so its walkable space no longer has to be modeled by hand. The bake is configured through [BakeSettings](../../../api/library/pathfinding/class.experimentalnavigationbakesettings_usc.md) of that node, and the result is written to the mesh asset its **Mesh Path** points at.
### Arguments

- *[NavigationMesh](../../../api/library/pathfinding/class.navigationmesh_usc.md)* **navigation_mesh** - Navigation mesh to be baked.

### Return value

Query to watch the bake with.
## ExperimentalNavigationBakeQuery engine.bake_navigation. bakeAsync ( NavigationMesh [] navigation_meshes )

Starts baking several [NavigationMesh](../../../api/library/pathfinding/class.navigationmesh_usc.md) nodes on the workers. One query reports the progress of the whole batch.
### Arguments

- *[NavigationMesh](../../../api/library/pathfinding/class.navigationmesh_usc.md)[]* **navigation_meshes** - Navigation meshes to be baked.

### Return value

Query covering the whole batch.
## ExperimentalNavigationBakeQuery engine.bake_navigation. bakeAsync ( int navigation_mask )
...
## int engine.bake_navigation. bakeForce ( ExperimentalNavigationMesh navigation_mesh )
...
## int engine.bake_navigation. bakeForce ( ExperimentalNavigationMesh [] navigation_meshes )
...
## bool engine.bake_navigation. bakeForce ( NavigationMesh navigation_mesh )

Bakes a [NavigationMesh](../../../api/library/pathfinding/class.navigationmesh_usc.md) node and blocks until it is done. The same conversion as in the asynchronous overload applies.
### Arguments
...
true if the mesh was baked; otherwise, false.
## bool engine.bake_navigation. bakeForce ( NavigationMesh [] navigation_meshes )

Bakes several [NavigationMesh](../../../api/library/pathfinding/class.navigationmesh_usc.md) nodes and blocks until the whole batch is done.
### Arguments
...
true if every mesh was baked; otherwise, false.
## int engine.bake_navigation. bakeForce ( int navigation_mask )
...
## int engine.bake_navigation. getNumWorkersAuto ( )
...
## void engine.bake_navigation. invalidateAsync ( WorldBoundBox bounds )
...
- *WorldBoundBox* **bounds** - Region that has gone out of date, in world coordinates.

## void engine.bake_navigation. invalidateAsync ( WorldBoundBox bounds , int navigation_mask )
...
## void engine.bake_navigation. invalidateForce ( WorldBoundBox bounds )
...
## void engine.bake_navigation. invalidateForce ( WorldBoundBox bounds , int navigation_mask )
