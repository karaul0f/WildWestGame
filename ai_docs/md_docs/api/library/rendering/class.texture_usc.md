...
Current Image format identifier (see the [FORMAT_*](../../../api/library/common/class.image_usc.md#FORMAT_ATI1) variables) or 0 if the specified texture format is unknown.
...
- *string* **name** - The friendly name of the texture used for GPU debugging (RenderDoc, NVIDIA Nsight).
...
**1** if the texture is valid; otherwise, **0**.
...
- *[Image](../../../api/library/common/class.image_usc.md)* **image** - Source image to be used for setting texture data.
...
- *int* **texture_format** - Texture format identifier (see the [TEXTURE_FORMAT_*](#FORMAT_ATI1) variables), or -1 - auto-detect.
...
- *[Image](../../../api/library/common/class.image_usc.md)* **image** - Source image to be used for setting the data of the 2D texture array layer.
...
Clears texture.
## int copy2D ( int dest_x = 0 , int dest_y = 0 , int src_x = 0 , int src_y = 0 , int w = -1 , int h = -1 )
...
## int copyCube ( int face , int dest_x = 0 , int dest_y = 0 , int src_x = 0 , int src_y = 0 , int w = -1 , int h = -1 )
...
## int create ( Image image , int flags = SAMPLER_FILTER_LINEAR , int skip_mipmaps = 0 , int format = -1 )

Texture constructor. Creates a new texture with the specified parameters.
### Arguments

- *[Image](../../../api/library/common/class.image_usc.md)* **image** - Image pointer.
...
## int create2D ( int width , int height , int format , int flags = SAMPLER_FILTER_LINEAR )
...
**1** if the texture was created successfully; otherwise, **0**.
## int create2DArray ( int width , int height , int num_layers , int format , int flags = SAMPLER_FILTER_LINEAR )
...
## int create3D ( int width , int height , int depth , int format , int flags = SAMPLER_FILTER_LINEAR )
...
## int createCube ( int width , int height , int format , int flags = SAMPLER_FILTER_LINEAR )
...
## int createCubeArray ( int width , int height , int num_layers , int format , int flags = SAMPLER_FILTER_LINEAR )
...
Destroys texture.
## int load ( string name , int flags = SAMPLER_FILTER_LINEAR )
...
- *int* **image_format** - Image format identifier (see the [IMAGE_FORMAT_*](../../../api/library/common/class.image_usc.md#FORMAT_ATI1) variables) or 0 if the specified texture format is unknown.
- *int* **flags** - Unsigned format flag. 1 to interpret integer image format as unsigned, 0 - as signed.

### Return value

Texture format identifier (see the [TEXTURE_FORMAT_*](#FORMAT_ATI1) variables) or 0 if the specified image format is unknown.
## int formatTextureToImage ( int texture_format )

Returns the image format corresponding to the specified texture format.
### Arguments

- *int* **texture_format** - Texture format identifier (see the [TEXTURE_FORMAT_*](#FORMAT_ATI1) variables).

### Return value

Image format identifier (see the [IMAGE_FORMAT_*](../../../api/library/common/class.image_usc.md#FORMAT_ATI1) variables) or 0 if the specified texture format is unknown.
## int typeTextureToImage ( int texture_type )

Returns the image type corresponding to the specified texture type.
### Arguments

- *int* **texture_type** - Texture type identifier (see the [TEXTURE_TEXTURE_*](#TEXTURE_2D) variables).

### Return value

Image type identifier (see the [IMAGE_*](../../../api/library/common/class.image_usc.md#IMAGE_2D) variables) or 0 if the specified texture type is unknown.
## void clearBuffer ( )
...
- *[Texture](../../../api/library/rendering/class.texture_usc.md)* **texture** - Source texture.
...
- *[Texture](../../../api/library/rendering/class.texture_usc.md)* **src** - Source texture.

### Return value

**1** if the data of the source texture is copied successfully; otherwise, **0**.
## int getVideoMemoryUsage ( int width , int height , int format , int num_mipmaps , int flags , int num_faces , int num_layers , int depth )
...
## int copyRegion ( Texture src , ivec3 src_coords , int src_level , ivec3 dest_coords , int dest_level , int width , int height , int depth )

Copies the specified region from the source texture.
### Arguments

- *[Texture](../../../api/library/rendering/class.texture_usc.md)* **src** - Source texture from which the data is to be copied.
...
**1** if the specified region was successfully copied from the source to the destination; otherwise, **0**.
## int setBlob ( Blob blob )

Sets texture data from the specified source blob.
### Arguments

- *[Blob](../../../api/library/common/class.blob_usc.md)* **blob** - Source blob containing texture data.
