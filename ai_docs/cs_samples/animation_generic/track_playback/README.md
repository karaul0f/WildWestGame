...
The **Tracker.cs** file provides a ready-to-use C# interface to the **Tracker** system, which the **TrackPlayback** component works through. If you want to reuse the sample in your own projects as is, copy the files together with the `tracker_wrapper.h` script from the `usc_code` folder that it relies on.