...
For each configuration, the sample shows the number of ready mesh objects along with the RAM and resident VRAM taken by their mesh data (queried per object via the *MeshRender* memory usage methods). This makes it easier evaluate the performance, memory layout, and behavior of each procedural mode in different runtime conditions.
