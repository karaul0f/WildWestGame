...
-In **Manual** mode, the texture is updated only when explicitly calling the *RenderToTexture()* method. This is used in the **WidgetClock** component, where the GUI (a digital clock) is re-rendered once per second, only when the displayed time changes.
